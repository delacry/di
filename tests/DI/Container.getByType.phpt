...
// nameless (autogenerated) services render as their concrete class, not "01"/"02"
$builder = new DI\ContainerBuilder;
$builder->addDefinition(null)->setType(Service::class);
$builder->addDefinition(null)->setType(Service2::class);

$nameless = createContainer($builder);

Assert::exception(
	fn() => $nameless->getByType(stdClass::class),
	Nette\DI\MissingServiceException::class,
	'Multiple services of type stdClass found: Service, Service2. To replace one, decorate the existing service; to keep both, give one an identity tag.',
);
