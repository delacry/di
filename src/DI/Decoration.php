...
use ReflectionType;
use function array_diff, array_keys, array_map, array_values, class_implements, class_parents, count, implode, is_a, sprintf, strcmp, usort;
...
final class Decoration
{
	public static function apply(ContainerBuilder $builder): void
	{
		...
		foreach ($slots as $slot) {
			...
			for ($i = 0, $n = count($chain) - 1; $i < $n; $i++) {
				...
				if ($param !== null && ($assigned[$outerName][$param] ?? null) !== $innerName) {
	...
	private static function findBase(array $definitions, string $type, ?string $tag, bool $optional): ?string
	{
		...
		if (count($found) > 1) {
			throw new ServiceCreationException(sprintf(
				...
				$type,
				$tag === null ? '' : sprintf(" with tag '%s'", $tag),
				implode(', ', array_map(static fn(string $name): string => self::describe($definitions[$name]), $found)),
			));
		}
	...
	/**
	 * Renders a service for an error message as its concrete class (suffixed with #tag
	 * for a non-default identity tag), falling back to its quoted name when the type is
	 * unknown — far clearer than an autogenerated name like "0179".
	 */
	private static function describe(Definition $def): string
	{
		$type = $def->getType();
		if ($type === null) {
			return "'" . $def->getName() . "'";
		...
		$tag = $def->getTag();
		return $tag === Definition::DefaultTag ? $type : $type . '#' . $tag;
	}
	 ...
	 *
	 * Returns null when the decorator has no parameter that can take the inner (or no
	 * constructor at all): it then fully *replaces* the slot rather than wrapping it — the base
	 * is still buried and the decorator heads the slot, but nothing is wired into it. This
	 * mirrors Symfony, where ignoring the decorated service is a complete replacement.
	 *
	 * @param  class-string  $type
	...
	private static function findInnerParam(ServiceDefinition $outer, string $type, ?string $innerType, ?string $slotTag, string $innerName, array $assigned): ?string
	{
		...
		if ($constructor === null) {
			return null; // no constructor → full replacement of the slot
		}
		...
		return $exact ?? $fallback; // null → no accepting parameter, so the decorator replaces the slot
	}
