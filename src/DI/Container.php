...
use function array_filter, array_flip, array_key_exists, array_keys, array_map, array_merge, array_search, array_values, class_exists, count, get_class_methods, implode, in_array, interface_exists, is_a, is_object, natsort, preg_match, sprintf, str_replace, ucfirst;
...
class Container
{
	...
	/**
	 * Renders service names for an error message: an autogenerated name (e.g. "0142") becomes
	 * the concrete class it resolves to, a given name is kept. A non-default identity tag is
	 * appended as #tag when the lookup had none, so differently-tagged candidates are told
	 * apart. Identical labels fall back to appending the name.
	 *
	...
	private function describeServices(array $names, bool $appendTag): array
	{
		...
		foreach ($names as $name) {
			$type = preg_match('#^0\d+$#D', $name) === 1 ? $this->serviceType($name) : null;
			$base = $type ?? $name;
			$tag = $this->serviceTags[$name] ?? Definition::DefaultTag;
	...
	/**
	 * The concrete class a service name resolves to (the most-derived type it is wired under),
	 * for error messages; null when it can't be determined, so the caller keeps the bare name.
	 */
	private function serviceType(string $name): ?string
	{
		$types = [];
		foreach ($this->wiring as $candidate => $slots) {
			foreach ($slots as $slot) {
				if (in_array($name, $slot, true)) {
					$types[] = $candidate;
					break;
				}
			}
		...
		foreach ($types as $type) {
			foreach ($types as $other) {
				if ($type !== $other && !is_a($type, $other, true)) {
					continue 2; // not a subtype of every other candidate → not the concrete one
				}
			}
